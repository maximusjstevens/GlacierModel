...
def glacier(D,b_in,delx,nxs,x,zb,bedprof,nyrs,spin,Hinit=None):
    ...
    fs = 5.0e-19*s_per_year                # sliding parameter fromOerlemans, page 62 (Pa^-3 m^2 yr-1). Units given in Oerlemans are per second
...
if __name__ == "__main__":
    ...
    savedir=os.path.join(spot,'resultsFolder/elahighramp')
    ...
    nyrs=30. #number of years to run simulation for. Explicit time stepping, so it takes a while if you want high-resolution space (e.g. 2 minutes for 25m grid spacing and 50-year model run on my macbook)
