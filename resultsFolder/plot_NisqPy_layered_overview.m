%%% model glacier animation (full glacier)
...
if vidrecord
    vidObj = VideoWriter('Nisq_full_12fps.avi');
...
clf;
% box on
set(fig20,'color','white');
set(fig20, 'Units', 'normalized')
% set(fig20, 'Position', [0 0 1 1])
set(fig20, 'Position', [0 0 1 1])
bga=gca;
...
for jj=1:r-1
    ...
    p1=axes('Position',[.06 .08 0.92 0.9],'Units','normalized');
    hold(p1,'on')
...
%     pos=[5000,8000,750,2400];
%     r1=rectangle('Position',pos);
%     r1.EdgeColor=[1 1 1];
...
%     
    ...
    %     elseif jj < 32 && jj > 17
    %         %         st='The Nisqually Glacier has been retreating at a rate of about 20 meters per year since 1980';
    %         st={'The glacier model does not reproduce or predict the glacier''s behavior exactly, but it does help scientists understand the various data from the glacier'};
    %         tt = annotation('textbox');
    %         tt.String=st;
    %         tt.FontSize = fstext;
    %         %         tt.FontWeight='bold';
    ...
    %     %%
    %
    %     p3a=axes('Position',[.57 .55 0.4 0.4]);
