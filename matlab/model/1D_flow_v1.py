...
xmx = 6500.0          # max of the domain size in m
delx = 50.0              # grid spacing in m
nxs = np.round(xmx/delx) + 1  # number of grid points
x = np.append(np.arange(0,xmx,delx),xmx)# x array (each point)

delt = 0.004 # time step in yrs 
ts = 0.0             # starting time in yrs
tf = 200.0      # final time in yrs
...
b = 7-(15/6000)*x      # mass balance in /yr     MAX: ARE THESE ARBITRARY UNITS? 

## ---------------------------------
# glacier bed geometries
#---------------------------------
delb = 4e3/log(3)
zb =  4392.*exp(-x/delb)     # bed profile in m  
#zb = (-0.1*x+1000)  
...
#-----------------------------------------
   for j in range nxs-1:  # 
       ...
       elif H(j)==0 && H(j-1)>0 # at the glacier toe
